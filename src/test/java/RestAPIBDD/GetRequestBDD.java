...
public class GetRequestBDD {
	...
	@Test
	public void test2() {
		RestAssured.given().baseUri("http://localhost:3000/employees").when().get("/3").then().log().body()
				.statusCode(200).body("name", Matchers.equalTo("James"));
	...
	public void test3() {
		...
		for (int i = 0; i < ids.size(); i++) {
			...
			if (ids.get(i) == empId) {
				Assert.assertEquals(names.get(i), "Veena");
			}
		}
	}
}
